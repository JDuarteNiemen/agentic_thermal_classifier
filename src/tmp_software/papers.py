...
def CreateLibrary(accession, meta, search_results=5):
    ...
    os.makedirs(f'TMP_results/accessions/{accession}/library/accession_lit', exist_ok=True)
    ...
    if pmids:
        for pmid in pmids:
            ...
            if not pmcid:  # No PMCID avaliable
                ...
                WritePaper(paper, f'TMP_results/accessions/{accession}/library/accession_lit/{pmid}.txt')
            if pmcid: # pmid converted to pmcid
                DownloadPaper(pmcid, f'TMP_results/accessions/{accession}/library/accession_lit/') # download the paper
    else:
        ...
        for pmid in pmids:
            ...
            if not pmcid: # not convertable
                ...
                WritePaper(paper, f'TMP_results/accessions/{accession}/library/accession_lit/{pmid}.txt')
            if pmcid:
                DownloadPaper(pmcid, f'TMP_results/accessions/{accession}/library/accession_lit/')

def HostLibrary(accession, host, search_results=5):
    os.makedirs(f'TMP_results/accessions/{accession}/library/host_lit', exist_ok=True)
    ...
    for pmid in pmids:
        ...
        if not pmcid:
            ...
            WritePaper(paper, f'TMP_results/accessions/{accession}/library/host_lit/{pmid}.txt')
        if pmcid:
            DownloadPaper(pmcid, f'TMP_results/accessions/{accession}/library/host_lit/')
